...
async function runAudit() {
  ...
  try {
    ...
    if (data.status !== 'success') throw new Error(`Citizen login failed: ${JSON.stringify(data)}`);
    ...
    data = await res.json();
    if (data.status !== 'success') throw new Error(`Ops login failed: ${JSON.stringify(data)}`);
    ...
    data = await res.json();
    if (data.status !== 'success') throw new Error(`CM login failed: ${JSON.stringify(data)}`);
