    ...
    cron.schedule('0 * * * *', async () => {
        ...
        try {
            ...
            for (const complaint of activeComplaints) {
                if (!complaint.category.slaConfig)
                    continue;
                ...
                if (new Date() > deadline) {
                    ...
                    if (escalationTarget === 'OPERATIONS') {
                        await NotificationService.notifyRoleInDistrict(complaint.districtId, 'OPERATIONS', 'SLA_BREACH', 'SLA Breached', `Complaint ${complaint.complaintNo} has breached its SLA.`);
                    }
                }
            }
...
//# sourceMappingURL=sla.job.js.map