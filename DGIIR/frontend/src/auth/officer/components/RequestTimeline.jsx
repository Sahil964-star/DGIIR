...
const RequestTimeline = ({ currentStatus = 'review' }) => {
  ...
  const states = [
    ...
    { id: 'review', label: 'Under Review', description: 'Operations validation' },
    { id: 'verified', label: 'Verified', description: 'Pending final sign-off' },
    { id: 'approved', label: 'Approved', description: 'Access granted' }
  ];
              ...
              <div key={state.id} className="flex flex-col items-center w-1/4">
                ...
                >
                  {isCompleted ? (
                    ...
                    state.id === 'review' ? <AlertCircle size={18} /> : <Clock size={16} />
                  ) : (
